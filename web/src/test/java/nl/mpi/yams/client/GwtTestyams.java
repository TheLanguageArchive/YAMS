...
import nl.mpi.yams.shared.FieldVerifier;
...
public class GwtTestyams extends GWTTestCase {
    ...
    public void testGreetingService() {
        ...
        SearchOptionsServiceAsync searchOptionsService = GWT.create(SearchOptionsService.class);
        ServiceDefTarget target = (ServiceDefTarget) searchOptionsService;
        target.setServiceEntryPoint(GWT.getModuleBaseURL() + "yams/search");
        ...
        searchOptionsService.getRootDataNodes("YAMS-DB", new AsyncCallback<List<SerialisableDataNode>>() {
            public void onFailure(Throwable caught) {
                // The request resulted in an unexpected error.
                fail("Request failure: " + caught.getMessage());
            }

            public void onSuccess(List<SerialisableDataNode> result) {
                // Verify that the response is correct.
                assertTrue(result != null);
                assertEquals(1, result.get(0).getChildList());
